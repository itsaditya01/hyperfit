...
export const mountainClimber = (poses, data) => {
    ...
    if (is_horizontal(poses, 4)){
        if(angle_lk>130 && angle_rk>130 && angle_lh>140 && angle_rh>140)
            {state=0}
        else if (angle_rh>100 && angle_lh<90
            && angle_lk<90 && angle_rk>90)
            {state=1}
        else if (angle_rh<90 && angle_lh>100
            && angle_lk>90 && angle_rk<90)
        {state=2}
    }
...
export const planks = (poses, data, extras, callback) => {
    ...
    const curr_seconds = new Date().getHours()*3600 + new Date().getMinutes()*60 + new Date().getSeconds()
    ...
    if(is_horizontal(poses,4)){
        ...
        if(angle_le<110 && angle_le>45 &&  angle_lh>127 &&  angle_lk>140 && angle_ls<100 && angle_ls>45){
            state=1
...
export const highKnees = (poses, data) => {
    ...
    if (visibleCoords({ poseLandmarks: poses })*100>=90){
        if (angle_lk_xy>120 && angle_rk_xy>120 && angle_lh_xy>130 && angle_rh_xy>130 && knee_ankle_distance(poses)){state=0}
        else if (angle_rk_xy>100 && angle_rh_xy>120 && angle_lh<120 && angle_lk<100 && knee_ankle_distance(poses)){state=1}
        else if(angle_lk>100 && angle_rh<120 && angle_lh_xy>120
            && angle_rk<100 && knee_ankle_distance(poses))
            {state=2}
    }
...
export const legRaise = (poses, data) => {
    ...
    const angle_lh=getAngleZ(poses[11],poses[23],poses[25])
    const angle_rh=getAngleZ(poses[12],poses[24],poses[26])
    if(visibleCoords({ poseLandmarks: poses })*100>=50 && is_horizontal(poses,4) && angle_lk>130 && angle_rk>130)
    {        
            if (angle_lh>140 && angle_rh>140 )
                {state=0}
            else if (angle_rh<120 && angle_lh<120)
                {state=1}
    }
...
export const sprawl = (poses, data) => {
    ...
    if (visibleCoords({ poseLandmarks: poses })*100>=50 ){
        if(is_horizontal(poses,4) && angle_le>130 && angle_lh>130 &&  angle_lk>140 && angle_ls>45)
            {state=0}
        else if(!is_horizontal(poses,4)&& angle_rh<120 && angle_lh<120 && angle_rk<90 && angle_ls>50 && angle_lk<90)
            {state=1}
    }
    if ((previous_state==1||previous_state==2)&&state==0){
        data.count++;
...
export const pushUps = (poses, data) => {
    ...
    if (is_horizontal(poses,2)){
        if( angle_le>130 && angle_lh>130 &&  angle_lk>130 && angle_ls>45)
            {state=0}
        else if(angle_le<90 && angle_lh>130 &&  angle_lk>130 && angle_ls>45)
            {state=1}
    }
...
export const squats = (poses, data, callback) => {
    ...
    if(((Math.abs(poses[0].y - poses[31].y)*720 >= window.innerHeight*.5) || (Math.abs(poses[0].y - poses[32].y)*720 >= window.innerHeight*.5)) && visibleCoords({ poseLandmarks: poses })*100 >= 80)
    {
            if (lk_angle>120 && rk_angle>120 && lb_angle>120 && rb_angle>120 && knee_position(poses))
                {state=0}
            else if (rb_angle<100 && lb_angle<100 && rk_angle<90 && lk_angle<90 && knee_position(poses))
                {state=1}
...
export const warrior1 = (poses, data, extras, callback) => {
    ...
    if(visibleCoords({ poseLandmarks: poses })*100 >= 70)
    {
        ...
        if((lk > 90 && lk < 150 && rk > 130 && ankles_apart(poses,1.6) && hand_condition) || ((rk > 90 && rk < 150 && lk > 130 && ankles_apart(poses,1.6) && hand_condition)))
        {
            // holdTime++
            warrior=true
            // data.count = true
...
export const warrior2 = (poses, data, extras) => {
    ...
    if(visibleCoords({ poseLandmarks: poses })*100 >= 70)
    {
        ...
        const hand_condition = ls>40 && rs>40 && ls<120 && rs<120 && le>140 && re>140
        ...
        if((lk > 90 && lk < 120 && rk > 140 && ankles_apart(poses,1.6) && hand_condition) || (rk > 90 && rk < 120 && lk > 150 && ankles_apart(poses,1.6) && hand_condition))
        {
            // holdTime++
            warrior=true
            // data.count = true
